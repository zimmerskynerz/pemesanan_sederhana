...
$cek_session = mysqli_query($conn, "select * from tbl_user where username='$session_id'") or die('Error In Session');
$data_member = mysqli_fetch_array($cek_session);
...
$keranjang_jml = mysqli_query($conn, "select count(*) as jml from rinci_transaksi as A INNER JOIN tbl_transaksi as B ON A.id_transaksi = B.id_transaksi where B.status='MENUNGGU' AND B.id_user='$id_user'");
